...
async function startTracking(device) {
  try {
    ...
    const response = await fetch(`http://3.92.51.134/devices/status/${device.imei}`);
    if (!response.ok) {
...
const cargarGeozonas = async () => {
  try {
    const response = await fetch('http://3.92.51.134/geozone/geozones');
    if (!response.ok) {
      throw new Error('Error en la respuesta de la API');
    }
    const data = await response.json();
...
const saveGeozone = async () => {
  ...
  try {
    const response = await axios.post('http://3.92.51.134/geozone/geozones', geozoneData);
...
const confirmCreateGeozona = async () => {
  ...
  try {
    ...
    const response = await axios.put(`http://3.92.51.134/devices/geozones/${selectedGeozone.value._id}`, geozoneData);
