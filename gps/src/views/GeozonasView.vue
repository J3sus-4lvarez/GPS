...
const cargarGeozonas = async () => {
  try {
    const response = await fetch('http://54.160.235.122/geozone/geozones');
    if (!response.ok) {
      throw new Error('Error en la respuesta de la API');
    }
    const data = await response.json();
...
const saveGeozone = async () => {
  ...
  try {
    const response = await axios.post('http://54.160.235.122/geozone/geozones', geozoneData);
...
const confirmCreateGeozona = async () => {
  ...
  try {
    ...
    const response = await axios.put(`http://54.160.235.122/devices/geozones/${selectedGeozone.value._id}`, geozoneData);
