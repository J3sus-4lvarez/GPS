...
</style>op="toggleDeviceSelection(device)" />
            {{ device.deviceName }}
          </li>
        </ul>
        <button @click="confirmCreateRoute" class="create-button">Crear Ruta</button>
...
const routeName = ref('');
let routeid = "";
let routeNamed = ""; // Definir routeName como una rerencia reactiva
...
const cargarRutas = async () => {
  try {
    const response = await fetch('http://3.92.51.134/routes');
    if (!response.ok) {
      throw new Error('Error en la respuesta de la API');
    }
    const data = await response.json();
...
const selectRoute = async (route) => {
  ...
  try {
    const response = await axios.get(`http://3.92.51.134/routes/get-route/${route._id}`);
...
const saveRoute = async () => {
  if (waypoints.length > 1) {
    ...
    try {
      const response = await axios.post('http://3.92.51.134/routes/save-route', { name: routeName.value, waypoints: route });
...
const confirmCreateRoute = async () => {
  try {
    ...
    // Hacer el PUT para actualizar el parámetro routeName de los dispositivos seleccionados
    const putResponse = await axios.put(`http://3.92.51.134/devices/update-route/${routeid}`, routeData);
    console.log('Dispositivos actualizados:', putResponse.data);
