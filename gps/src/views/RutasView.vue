...
const cargarRutas = async () => {
  try {
    const response = await fetch('http://18.223.186.201/routes');
    if (!response.ok) {
...
const selectRoute = async (route) => {
  ...
  try {
    const response = await axios.get(`http://18.223.186.201/routes/get-route/${route._id}`);
...
const saveRoute = async () => {
  if (waypoints.length > 1) {
    ...
    try {
      const response = await axios.post('http://18.223.186.201/routes/save-route', { name: routeName.value, waypoints: route });
...
const confirmCreateRoute = async () => {
  try {
    ...
    const putResponse = await axios.put(`http://18.223.186.201/devices/update-route/${routeid}`, routeData);
